...
/**
 * Le verset qu'une route API doit traiter, lu dans la source verifiee a
 * partir de la SEULE reference envoyee par le navigateur.
 *
 * POURQUOI LES ROUTES NE LISENT PLUS `arabic` DANS LA REQUETE
 * /api/tafsir et /api/hint mettaient leur reponse en cache sous la cle
 * « sourate:verset », mais construisaient le prompt avec le texte arabe envoye
 * par le client. N'importe quel utilisateur connecte pouvait donc envoyer
 * { sourate_num: 1, verse_num: 1, arabic: "…n'importe quoi…" } : la reponse de
 * l'IA etait rangee comme le tafsir d'Al-Fatiha et servie pendant une heure a
 * tous ceux qui ouvraient ce verset. Constate le 10 septembre 2026.
 *
 * Le texte coranique ne transite plus par le navigateur : seule la reference
 * le fait, et elle est validee contre le nombre de versets de chaque sourate.
 *
 * @returns {ReturnType<typeof versetAuthentique>} null si la reference est
 *   absente ou n'existe pas — l'appelant repond alors 400.
 */
export function versetDeLaRequete(body) {
  if (!body) return null
  return versetAuthentique(body.sourate_num, body.verse_num)
}

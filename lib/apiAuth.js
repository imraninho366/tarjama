 ...
 * Les routes API construisent leur client avec la cle anonyme, sans jeton.
 * Pour PostgreSQL elles sont donc un visiteur anonyme, exactement comme un
 * navigateur non connecte : `auth.uid()` y vaut null, et toute politique RLS
 * ecrite en fonction de l'utilisateur les rejetterait. C'est ce qui rendait
 * impossible de proteger la table `duels` — la verrouiller aurait casse la
 * route en meme temps que la fuite.
 *
 * En transmettant le jeton deja verifie par requireUser, la base retrouve
 * l'identite de l'appelant et les politiques deviennent applicables.
 *
 * Le jeton n'accorde rien de plus que ce que son porteur possede deja : il est
 * emis par Supabase pour cet utilisateur, et la base le revalide.
 */
export function clientDeLAppelant(req) {

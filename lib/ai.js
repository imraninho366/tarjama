 ...
 *
 * Ordre retenu, mesuré en production le 6 septembre 2026 :
 *
 *   groq       — 30 req/min, 8 K tokens/min, 200 K tokens/jour. Le plus rapide
 *                en latence (~300 ms contre ~1 s ailleurs).
 *   gemini     — 10 req/min mais 250 K tokens/MIN : de loin le plus gros débit
 *                en tokens, et le meilleur du lot sur l'arabe. En contrepartie
 *                il renvoie souvent un 503 « surchargé » — mesuré une fois sur
 *                deux en septembre 2026. Utile, mais pas un pilier.
 *   cohere     — 1 000 appels/MOIS, 20 req/min. Rapide (~400 ms) mais petit.
 ...
 *   mistral    — inactif : accès API fermé sans carte bancaire. Voir le
 *                commentaire de son entrée, tout en bas de la liste.
 */
const PROVIDERS = [
  {
    name: 'groq',
  ...
  },
  {
    ...
    // gemini-2.5-flash renvoyait un 404 en septembre 2026 : Google l'a retiré
    // du service avant même sa date d'arrêt annoncée (16 octobre 2026). Un nom
    // de modèle est une donnée périssable, jamais une constante — si cette
    // entrée se met à échouer, comparer d'abord avec la liste à jour sur
  ...
  },
  {
    ...
    // Le catalogue gratuit d'OpenRouter change en permanence : llama-3.3-70b
    // y était gratuit, il est passé payant en septembre 2026. Gemma est choisi
    // pour le multilingue — Tarjama mélange français et arabe dans la même
    // requête, ce qui élimine les petits modèles anglophones.
    //
    // POUR TROUVER UN REMPLAÇANT, l'API publique liste l'état réel du jour et
    ...
    modelFast: 'google/gemma-4-31b-it:free',
  },
  {
    name: 'mistral',
    envKey: 'MISTRAL_API_KEY',
    url: 'https://api.mistral.ai/v1/chat/completions',
    // EN DERNIER, ET C'EST DÉLIBÉRÉ : ce compte rejette 100 % des appels par
    // un 429, y compris sur mistral-small-2603 que la console liste pourtant
    // comme accordé. La cause n'est ni la clé ni le nom du modèle — les deux
    // ont été testés — mais l'accès API lui-même, fermé tant que la bascule
    // « Pay-As-You-Go » n'est pas activée, ce qui réclame une carte bancaire.
    // Tarjama doit rester entièrement gratuite : on ne l'activera pas.
    //
    // L'entrée reste là parce qu'elle ne coûte rien en dernière position et
    // qu'elle repartira toute seule le jour où l'accès s'ouvrira. La placer
    // en tête, elle, faisait payer 220 ms perdues à CHAQUE requête.
    //
    // Ne pas remettre « mistral-small-latest » : cet alias ne figure pas dans
    // les modèles du compte. Les noms exacts sont sur
    // https://admin.mistral.ai/plateforme/limits
    model: 'mistral-small-2603',
    modelFast: 'mistral-small-2603',
  },
]
